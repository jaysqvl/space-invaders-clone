...
public class Frame extends javax.swing.JFrame implements Runnable, KeyEventDispatcher, MouseListener, MouseMotionListener, ActionListener {
    ...
    ArrayList<laser> laserArrayP = new ArrayList<laser>(); // laser Array from player
    ArrayList<laser> laserArrayE = new ArrayList<laser>(); // laser array from enemy
    ...
    public int numEnemiesX=8; //# Collumns
    public int numEnemiesY=4; //# Rows
    int totalEnemies=numEnemiesX*numEnemiesY; //total enemies in game
    boolean shiftDown=false;
    public boolean enableShoot=true; //Delay for player to shoot one shot
    public boolean enableShootE=true; //Delay for enemies to randomly shoot
    int objectRadius=30;
    int delay=0; //Start delay counter for when player is able to shoot again
    ...
    //Custom game loop 
    //(Created watching a video about the concept of gaming looping and the most effective implementations so not entirely my idea but it's a modified version of the original Minecraft version)
    ...
    public void myDrawCode(Graphics g) {
        ...
        //Draws the lasers (if they exist) and from the middle of the laser
        ...
        //Draws the lasers from the enemy (if they exist) and from the middle of the laser
        ...
        //Draws the player wherever his position his
    ...
    public Image enemyImage() {
        String enemyDir=(currentDir + "/enemy30.png");
        try {
        enemyImage = ImageIO.read(new File(enemyDir));
        } catch (IOException e) {
            System.out.println("Error reading file enemy!");
        }
        return enemyImage;
    }
    
    public Image laserImage() {
        String laserDir=(currentDir + "/laser10.png");
        try {
            laserImage = ImageIO.read(new File(laserDir));
        } catch (IOException e) {
            System.out.println("Error reading file laser!");
        }
        return laserImage;
    }
    
    public Image playerImage() {
        String playerDir=(currentDir + "/ship30.png");
        try {
            playerImage=ImageIO.read(new File(playerDir));
        } catch (IOException e) {
            System.out.println("Error reading file player!");
        }
        return playerImage;
    }
    ...
    //fills the enemy array with objects of the type enemy class
    //Should only do this once at the start of the game
    public void fillEnemies() {
        //Initial separation distance between each enemy
        int separateX=55;
        int separateY=50;
        int initialX=40;
        int startX=40;
        ...
        //For each enemy, there must be a rectangle at its position to check to see if the enemy got hit
